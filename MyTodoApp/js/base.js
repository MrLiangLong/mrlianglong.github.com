...
;(function(){
    'use strict';
    /*‘；’结束之前的程序，防止前面文字压缩等发生错误，
     * 此匿名函数意思：用一种更严格的模式来运行程序
     * 防止这种错误：var a=1 ;console.log('window.a',window.a);防止污染，把a变量变成window的属性
     * */
    var $form_add_task=$('.add-task');
    var $body=$('body');
    var $window=$(window);
    var $task_delete=$('.action.delete');
    ...
    function render_task_detail(index){
        ...
        var item=task_list[index];
         var tpl=
             ...
             '<label>提醒时间</label>'+
             '<input id="datetimepicker" name="remind_data" type="text" value="'+(item.remind_data?item.remind_data:"")+'"/>'+
             '</div>'+
...
/*        for(var k in new_task){
   ...
   /* 渲染全部的task模板*/
    function render_task_list(){
        ...
        for(var i=0;i<task_list.length;i++){
            ...
            if(item && item.complete){
                ...
                $task_list.prepend($task);//将元素加到某元素的第一条
            }
        }
    ...
    function pop(arg){
        ...
        '<div>' +
        '<button style="margin-right: 6px;" class="primary confirm">确定</button>' +
        ...
        $mask=$('<div style="background: rgba(0,0,0,.5)"></div>')
            .css({
                position:'fixed',
...
/*        $mask.on('click',function(){
        ...
        function adjust_box_position(){
            ...
            move_y=(window_height-box_height)/2-116;
